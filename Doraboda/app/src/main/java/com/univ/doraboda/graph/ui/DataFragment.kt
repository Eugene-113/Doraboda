package com.univ.doraboda.graph.ui
...
import com.univ.doraboda.graph.viewmodel.DataViewModel
...
class DataFragment : BaseFragment<FragmentDataBinding>() {
    ...
    override fun layoutInit() {
                ...
                viewModel.state.collect{
                    if(it.isLoading){
                        ...
                        if(it.isError){
                            ...
                            Glide.with(requireContext()).load(glideImage)
                                .diskCacheStrategy(DiskCacheStrategy.NONE)
                                .skipMemoryCache(true)
                                .into(binding.dataImageView)
                        }
                    }
                }
            }
        ...
        calendar2.set(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH), calendar.getActualMaximum(
            Calendar.DATE), 0, 0, 0)
        ...
        viewModel.handleIntent(DataViewModel.DataIntent.LoadBetweenEmotions(calendar1.timeInMillis, calendar2.timeInMillis))
        ...
        parentFragmentManager.setFragmentResultListener("calendarResult", this){ _, bundle ->
            ...
            calendar4.set(selectedCalendarItem.get(Calendar.YEAR), selectedCalendarItem.get(Calendar.MONTH), calendar.getActualMaximum(
                Calendar.DATE), 0, 0, 0)
            viewModel.handleIntent(DataViewModel.DataIntent.LoadBetweenEmotions(calendar3.timeInMillis, calendar4.timeInMillis))
        ...
        binding.dataTextView2.setOnClickListener {
            DateDialogFragment().apply {
                arguments = bundleOf(
                    "fragmentDate" to "${selectedCalendarItem.get(Calendar.YEAR)}/${
                        selectedCalendarItem.get(Calendar.MONTH) + 1
                    }"
                )
