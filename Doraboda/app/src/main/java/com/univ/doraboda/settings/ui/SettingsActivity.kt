package com.univ.doraboda.settings.ui
...
import com.univ.doraboda.settings.viewmodel.SettingsViewModel
import com.univ.doraboda.app.dialog.FileQuestDialogFragment
import com.univ.doraboda.app.dialog.LabelDialogFragment
...
class SettingsActivity : AppCompatActivity() {
    ...
    val startForResult2: ActivityResultLauncher<Intent> = registerForActivityResult(
        ...
        if(result.resultCode == RESULT_OK){
            ...
            viewModel.handleIntent(SettingsViewModel.SettingsIntent.LoadAllDoraData(uri))
        }
    }
    ...
    val startForResult1: ActivityResultLauncher<Intent> = registerForActivityResult(
        ...
        if(result.resultCode == RESULT_OK){
            val uri = result.data?.data ?: return@registerForActivityResult
            viewModel.handleIntent(SettingsViewModel.SettingsIntent.SaveAllDoraData(uri))
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportFragmentManager.setFragmentResultListener("settingsResult", this){ _, bundle ->
            ...
            viewModel.handleIntent(SettingsViewModel.SettingsIntent.SetLabelType(labelType))
        }
        ...
        binding.settingsSwitch.setOnCheckedChangeListener { _, isChecked ->
            viewModel.handleIntent(SettingsViewModel.SettingsIntent.SetQuoteMode(isChecked))
        }
        binding.settingsLayout4.setOnClickListener {
            val url = "https://github.com/Eugene-113/Doraboda"
            val intent = Intent(Intent.ACTION_VIEW, Uri.parse(url))
            startActivity(intent)
        }
        viewModel.handleIntent(SettingsViewModel.SettingsIntent.LoadAllFirst)
    }

    fun readFolder(){
        val calendar = Calendar.getInstance()
        val dateString = "${calendar.get(Calendar.YEAR)}/${calendar.get(Calendar.MONTH)+1}/${calendar.get(
